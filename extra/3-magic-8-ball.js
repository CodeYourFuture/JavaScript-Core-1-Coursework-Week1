...
function shakeBall() {
  ...
  // picks a random array from our multidimensional array by using the Math.Floor function with the Math.random * array.length. This makes a number between 0 and the array.length -1.
  const randomArray =
    answersArray[Math.floor(Math.random() * answersArray.length)];

  // we use that same math down here, but this time to select a specific index from the chosen inner array. These Math functions pick a number between 0 and the innerArray.length -1
  const randomElement =
...
function checkAnswer(answer) {
  // different checks for what array includes our string. Checking each inner array
  if (answersArray[0].includes(answer)) {
    return "very positive";
  } else if (answersArray[1].includes(answer)) {
    return "positive";
  } else if (answersArray[2].includes(answer)) {
    return "negative";
  } else if (answersArray[3].includes(answer)) {
    return "very negative";
  } else {
    return `Value not in any array: ${answer}`;
  }
}
